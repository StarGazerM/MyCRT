...
async def async_ssh_handler(client, mode):
    ...
    try:
        ...
        if mode == 'frontend':
            zmq_socket.connect('ipc://' + username.decode())
        else:
            chan.settimeout(10)
    ...
    while True:
        ...
        if len(data) is not 0:
            try:
                ...
                sock.send('offline already!!!' + str(err))
...
async def run_server(mode):
    ...
    while True:
        ...
        print('accepted')
        try:
            loop.create_task(async_ssh_handler(client, mode))
        except Exception as e:
...
if __name__ == '__main__':
    ...
    try:
        MODE = sys.argv[-1]
        print(sys.argv[-1])
        if MODE != 'frontend' and MODE != 'backend':
            raise getopt.GetoptError('参数不存在')
    except getopt.GetoptError as err:
        print(str(err))
        sys.exit(1)
    loop.create_task(run_server(MODE))
    loop.run_forever()
